...
if (transformer.result.errors.length === 0) {
  ...
  fs.writeFileSync('./output/ngsFinalReportTransformed.xml', transformer.result.content)
}
...
if (transformer.result.errors.length === 0) {
  ...
  fs.writeFileSync('./output/ngsFinalReportTransformed.json', JSON.stringify(JSON.parse(transformer.result.content), null, 2))
}
